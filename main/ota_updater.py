...
class OTAUpdater:
    ...
    def get_latest_version(self):
        ...
        if latest_release.status_code != 200:
            raise ValueError('GitHub API error {}: no latest release found'.format(latest_release.status_code))
        gh_json = latest_release.json()
        if 'tag_name' not in gh_json:
            raise ValueError('GitHub API: tag_name missing. Response: {}'.format(gh_json))
